...
class BaseIndicatorsUtils:
    ...
    def rsi(self, period=14, signal_period=14):
        if self.mode == 'custom':
            ...
            return rsi, rsi_ema.tolist()
        else:
            rsi_series = self.RSIIndicator(close=self.df['close'], window=period).rsi()
            rsi_list = rsi_series.tolist()
            # No modo padrão, também usamos o teu ema_list para manter a coerência
            rsi_ema = self.ema_list(rsi_list, signal_period)
            return rsi_list, rsi_ema.tolist()
    ...
    def macd(self, fast_period=12, slow_period=26, signal_period=9):
        if self.mode == 'custom':
            ...
            # O Histograma é a diferença entre a linha MACD e a linha de Sinal
    ...
    def double_rsi(self, rsi_slow_len=21, rsi_fast_len=5, atr_fast_len=7, atr_slow_len=24, smooth_len=3):
        ...
        for i in range(max(2, atr_slow_len), n):
            ...
            # 2. FILTRO DE ESTICAMENTO (OVEREXTENSION)
    ...
    def double_rsi_(self, rsi_slow_len=21, rsi_fast_len=5, atr_fast_len=7, atr_slow_len=24, lookback=14):
        ...
        for i in range(max(2, atr_slow_len), n):
            ...
            trend_down = is_trending_down and closes[i] < ema25[i] and ema25[i] < ema50[i] < ema100[i]

            is_adx = adx[i] > 20

            if rsi_slow[i] >= 60:
                overbought_slow = True
                oversold_slow = False
            elif rsi_slow[i] <= 40:
                overbought_slow = False
                oversold_slow = True
